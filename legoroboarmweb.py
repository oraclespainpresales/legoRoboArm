...
class RESTServer(Thread):
    ...
    def __init__(self, _roboarm):
        ...
        add_url(self.Start, '/move_start/')
        add_url(self.Stop, '/move_stop/')
        add_url(self.Initialize, '/initialize/')
    ...
    def run(self):
        ...
        try:
            ...
            logger.error('Could not START REST API web server ' + str(sys.exc_info()[0]))
            exit(-1)

    class Start:

        # def GET(self):
        #     logger.debug("move_start received!")
        #     roboarm.start_infinite_movement()
        #     return None
    ...
    class Stop:
        ...
        def POST(self):
            logger.debug("move_stop received!")
    ...
    class Initialize:
        ...
        def POST(self):
            logger.debug("initialize received!")
            roboarm.initialize()
            return None
...
class LegoRoboArm:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.rest_server.daemon = True
            self.rest_server.start()
            time.sleep(25)
            sound.speak("Web Server Ready!")
        except:
            logger.fatal("REST API not available")
            sys.exit(-1)
...
if __name__ == "__main__":
    try:
        logger.debug("Creating Robo Arm Object")
        ra = LegoRoboArm()
        try:
            ...
            ra.shutdown_roboarm()
            time.sleep(1)
            logger.fatal(str(sys.exc_info()[0]))
            sys.exit(-1)
    except:
        time.sleep(1)
